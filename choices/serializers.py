...
class Level6_Serializer(serializers.ModelSerializer):
    class Meta:
        model=Level6
        fields=['id','code','point','college','title','choice', 'courseInformation', 'order_number']
...
class Apprentice_Serializer(serializers.ModelSerializer):
    class Meta:
        model=Apprentice
        fields=['id','name','level','company','choice', 'courseInformation', 'order_number']
        extra_kwargs = {'choice':{'allow_null': True,'required': False}}
    def create(self, validated_data):
        student = self.context['request'].user.student
        try:
...
class Level5_Serializer(serializers.ModelSerializer):
    class Meta:
        model=Level5
        fields=['id','code','college','title','choice', 'courseInformation', 'order_number']
        extra_kwargs = {'choice':{'allow_null': True,'required': False} }
    def create(self, validated_data):
        student = self.context['request'].user.student
        try:
